...
When building MoneyDrive admin screens or analytics pages, apply this skill with the MoneyDrive dark palette:
- Sidebar background: `bg-card` (navy dark)
- KPI cards: `bg-card` with burnt orange accent numbers
- Chart fills: gold (`--color-secondary`) for primary, burnt orange (`--color-primary`) for secondary
- Metric labels in SA English: "Gross Earnings", "Active Drivers", "In-Car Sales", "Nduna Responses"
- Always include a pilot mode indicator badge if `pilotMode === true`
