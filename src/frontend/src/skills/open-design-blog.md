...
MoneyDrive blog posts are targeted at SA ride-hailing drivers and gig economy workers.

**Voice & tone**: Direct, street-smart, SA-localized. Use "driver" not "user". Reference local context: load-shedding (Eskom outages), petrol price changes, e-hailing platform commissions.

**Typical topics**:
- "How a Cape Town Bolt driver made R12,000 extra last month with in-car sales"
- "5 ways to close your first advertising deal using Nduna"
- "MiniPay explained: save in USDC when the rand drops"
- "Why Tier 2 MoneyDrive drivers earn 34% more than Tier 1"
- "Fleet owners: how to track income across 3 vehicles without a spreadsheet"

**Author attribution**: Use "The MoneyDrive Team" or "Nduna" (for AI-generated insights posts).

**Pull quote accent**: Burnt orange left border (`--color-primary`)

**Hero image gradient**: Use burnt orange → navy gradient (`--color-primary` → `--color-background`) for hero placeholder when no image is available.

**Related posts**: Always end with 3 related articles to increase session depth.
